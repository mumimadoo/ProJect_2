...
for met in metrics:
    ...
    for m_name, model in models.items():
        ...
        while len(remaining_features) > 0:
            ...
            if best_new_score >= 0.85 or best_new_score <= best_score_for_model:
                if best_new_score >= 0.85:
...
for met in metrics:
    print(f"\t{met}")
    for n_comp in [0.90]: # ลองหลายจำนวน Components เพื่อหาตัวที่ Final Score ดีที่สุดรายโมเดล
        ...
        models = get_models()
        for m_name, model in models.items():
            base = get_base_score(model, X_tr_pca, X_te_pca, met)
            final = calculate_custom_score(base, n_comp)
            # ในที่นี้เราจะบันทึกเฉพาะตัวที่ให้ผลลัพธ์ดีที่สุดของ PCA รายโมเดลนั้นๆ
            results_list.append({'Method': f'PCA-{n_comp}', 'Metric': met, 'Model': m_name, 'Score': final})
    
    # แสดงผลสรุป PCA ของ Metric นั้นๆ (เลือก n_comp ที่ดีที่สุดมาโชว์)
    temp_df = pd.DataFrame(results_list)
    for m_name in models.keys():
        best_pca = temp_df[(temp_df['Metric']==met) & (temp_df['Model']==m_name) & (temp_df['Method'].str.contains('PCA'))].sort_values(by='Score', ascending=False).iloc[0]
        print(f"\t\t- {m_name:22} | Final: {best_pca['Score']:.4f} | ใช้ฟีเจอร์ ({best_pca['Method'].split('-')[1]} Components)")
...
for met in metrics:
    ...
    print(f"BEST {met.upper():10}: {best['Score']:.4f} | Model: {best['Model']:18} | Method: {best['Method']}")